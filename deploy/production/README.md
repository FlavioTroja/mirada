...
| le anteprime su WhatsApp/Telegram non mostrano nulla, e ogni scheda evento condivide il titolo del SITO | `curl -s https://mirada.dance/eventi \| grep -c ng-server-context` → deve dire **1** | l'SSR non sta avvenendo: il proxy manda `X-Forwarded-*` e `www` non è autorizzato a fidarsene, quindi ripiega sul guscio da rendere nel browser — che i crawler non eseguono. Si corregge in `www/src/server.ts` (`trustProxyHeaders`) e serve un rilascio dell'immagine. ⚠️ Dal loopback (`curl http://127.0.0.1:8082/eventi`) la resa funziona lo stesso: la differenza si vede **solo** passando dal dominio |
