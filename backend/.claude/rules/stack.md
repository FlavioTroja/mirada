...
5. Auth: JWT via `@fastify/jwt`, enforced by `@middleware/Authenticate` and `@middleware/HasPermission`. Sessions are opened **either** by password **or** by OpenID Connect against Authentik — both end at the same signed payload. See `authentication.md`.
